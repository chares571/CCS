...
<section class="panel">
    ...
    <form method="POST" action="{{ route($prefix.'.announcements.store') }}" enctype="multipart/form-data" class="js-announcement-form">
        @csrf
        ...
        <div class="alert alert-warning js-autosave-banner" hidden>
            Unsaved draft found on this device.
            <button type="button" class="btn btn-success" data-autosave-restore>Restore</button>
            <button type="button" class="btn btn-danger" data-autosave-discard>Discard</button>
        </div>

        <label>Title</label>
        <input type="text" name="title" value="{{ old('title') }}" required class="announcement-title-input">
        ...
        <div class="action-inline mt-10">
            <input type="hidden" name="content_format" value="{{ old('content_format', 'markdown') }}">
            <div style="min-width: 240px;">
                <label>Schedule Publish Time (optional)</label>
                <input type="datetime-local" lang="en-US" name="publish_at" value="{{ old('publish_at') }}">
            </div>
            <div style="align-self: end;">
                <label class="subnav-link" style="cursor:pointer;">
        ...
        <div class="action-inline action-inline--equal mt-10">
            ...
            <button class="btn btn-success" type="button" data-publish>Publish</button>
        </div>
...
(() => {
    ...
    const autosaveBanner = form.querySelector('.js-autosave-banner');
    const restoreBtn = form.querySelector('[data-autosave-restore]');
    const discardBtn = form.querySelector('[data-autosave-discard]');
    ...
    const storageKey = 'ccs.announcements.autosave.new';
    let dirty = false;
    ...
    const applyAutosave = (payload) => {
        ...
        if (draft) draft.checked = !!payload.is_draft;
        if (caption) caption.value = payload.image_caption || '';
        refreshToolbar();
    };

    const autosaved = loadAutosave();
    const isEmpty = !(title?.value || '').trim() && !((contentTextarea?.value || '').trim());
    if (autosaved && isEmpty && autosaveBanner) {
        autosaveBanner.hidden = false;
        restoreBtn && restoreBtn.addEventListener('click', () => {
            applyAutosave(autosaved);
    ...
    [title, contentTextarea, publish, draft, caption, format].forEach((el) => el && el.addEventListener('input', scheduleAutosave));
    draft && draft.addEventListener('change', scheduleAutosave);
